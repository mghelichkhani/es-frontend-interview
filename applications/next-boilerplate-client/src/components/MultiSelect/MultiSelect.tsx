...
import { ChevronDownIcon, ChevronUpIcon, MagnifyingGlassIcon, CheckIcon, Cross2Icon } from '@radix-ui/react-icons'
...
export default function MultiSelect({
  ...
  const groupId = useRef(`multiselect-group-${Math.random().toString(36).substr(2, 9)}`).current
  const buttonId = useRef(`multiselect-button-${Math.random().toString(36).substr(2, 9)}`).current
  const searchInputId = useRef(`multiselect-search-${Math.random().toString(36).substr(2, 9)}`).current
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Enter') {
        ...
        const isCheckbox = target.getAttribute('role') === 'checkbox' || target.closest('[role="checkbox"]')
        const isLabel = target.tagName === 'LABEL' && target.closest(`#${groupId}`)
      ...
      <Popover.Root open={open} onOpenChange={handleOpenChange}>
        <Popover.Trigger asChild>
          <Button
            ...
            iconAfter={open ? <ChevronUpIcon className="h-5 w-5" /> : <ChevronDownIcon className="h-5 w-5" />}
          >
        ...
        <Popover.Portal>
          ...
          <div 
            ...
            className={error ? "mt-2 max-h-64 overflow-auto rounded-md" : "max-h-64 overflow-auto rounded-md"}
          >
            ...
            {!isLoading && !error && !localQ && (
              <>
                <MultiSelectOption
                  ...
                  label={allOnPageSelected ? t('common.deselectAll') : t('common.selectAll')}
                  ariaLabel={allOnPageSelected ? t('common.deselectAll') : t('common.selectAll')}
            ...
            {!isLoading && !error && filtered.length === 0 && (
              <div className="px-3 py-2 text-[#6B7280]">{t('common.noResults')}</div>
            )}
            ...
            {!isLoading && !error && filtered.map(o => {
              ...
              return (
                <MultiSelectOption
                  ...
                  label={o.label}
                  ariaLabel={t('multiSelect.selectItem', { label: o.label })}
                />
              )
            })}
          </div>
